package org.apache.ignite.enums;
...
public enum GlobalEnum {
    ...
    DataBaseNotSupport("数据源:%s,暂时不支持!"),
    ...
    RequestParamEmptyByField("请求参数:%s不能为空!"),
    ...
    FrequentlyLogin("账号\"%s\"尝试登陆\"%s\"次,帐户名或登录密码不正确,将锁定%s小时或联系管理员!"),
    ...
    ApiQueryError("第三方接口查询配置无效或数据无效"),
    ...
    ApiMethodOrNameByWebServiceEmpty("第三方接口:'%s',方法名、命名空间URI均不能为空"),
    ...
    DeleteInterfaceUsed("删除不被允许，已经被:'%s'所引用"),
    ...
    DataBaseOperationFailed("执行数据库操作失败"),

    ApiNoSubscript("API未订阅"),

    TokenOvertimeForClient("token信息已经过期,请重新获取!"),

    ApiOvertime("api订阅已经过期,请重新获取!"),

    ApiPublishOvertime("API已经失效，请联系API发布者"),

    ApiCanNoDown("接口已经被订阅,不允许下架"),

    AccessKeyOrSecretKeyEmpty("accessKey或secretKey为空!"),
